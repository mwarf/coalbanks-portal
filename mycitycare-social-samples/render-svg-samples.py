...
def lines(items, x, y, size, fg, weight=800, gap=1.05, anchor="start"):
    spans = "".join(
        f'<tspan x="{x}" dy="{0 if i == 0 else int(size * gap)}">{esc(line)}</tspan>'
        for i, line in enumerate(items)
    )
    return (f'<text x="{x}" y="{y}" fill="{fg}" font-family="{FONT}" '
...
<text x="96" y="1235" fill="{fg}" font-family="{FONT}" font-size="34" font-weight="800" letter-spacing="-1">mycitycare</text>
...
def card(filename, pair, eyebrow, headline, body, footer, kind="event", stat=""):
    ...
    if kind == "stat":
        content = (f'<text x="96" y="700" fill="{fg}" font-family="{FONT}" font-size="400" '
                   f'font-weight="900" letter-spacing="-18">{esc(stat)}<tspan fill="{accent}">.</tspan></text>'
                   + lines(body, 96, 835, 40, fg, 400, 1.45))
    elif kind == "quote":
        content = (f'<text x="96" y="430" fill="{accent}" font-family="{FONT}" font-size="250" font-weight="900">“</text>'
...
def slide(filename, pair, i, total, role, label, headline, body=None, footer="", stat=""):
    ...
    body = body or []
    if role == "stat":
        content = (f'<text x="96" y="690" fill="{fg}" font-family="{FONT}" font-size="300" '
                   f'font-weight="900">{esc(stat)}<tspan fill="{accent}">.</tspan></text>'
                   + lines(body, 96, 830, 38, fg, 400, 1.45))
    elif role == "quote":
        content = (f'<text x="96" y="430" fill="{accent}" font-family="{FONT}" font-size="240" font-weight="900">“</text>'
